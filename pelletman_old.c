...
void update_game(player *p1, pellet *pellets) {
    ...
    // Update player position
        for (int i = 0; i < p1->length; i++) {
            if (i > 0) {
                p1[i].oldpos[0] = p1[i].segment.x;
                p1[i].oldpos[1] = p1[i].segment.y;
                p1[i].segment.x = p1[i - 1].oldpos[0];
                p1[i].segment.y = p1[i - 1].oldpos[1];
    
            } else {
                p1[i].oldpos[0] = p1[i].segment.x;
                p1[i].oldpos[1] = p1[i].segment.y;
                p1[i].segment.x += p1->character_size * dirx;
                p1[i].segment.y += p1->character_size * diry;
                // Bounds checking. Does not handle corner interactions well.
                if (p1[i].segment.x > width - p1->character_size) {
                    p1[i].segment.x = width - p1->character_size;
                    set_player_direction(UP, p1);
                } else if (p1[i].segment.x < 0) {
                    p1[i].segment.x = 0;
                    set_player_direction(DOWN, p1);
                } else if (p1[i].segment.y > height - p1->character_size) {
                    p1[i].segment.y = height - p1->character_size;
                    set_player_direction(LEFT, p1);
                } else if (p1[i].segment.y < 0) {
                    p1[i].segment.y = 0;
                    set_player_direction(RIGHT, p1);
                }
            }
            if (SDL_HasIntersection(&p1->segment, &p1[i+1].segment)) {
                    printf("Collision with self\n");
                    game = 0;
                    break;
                }
        ...
        for (int i = 0; i < pellet_count; i++) {
            if (SDL_HasIntersection(&p1->segment, &pellets[i].rect) && pellets[i].active == true) {
                ...
                p1[p1->length].segment = p1[p1->length - 1].segment;
                p1->length++;
